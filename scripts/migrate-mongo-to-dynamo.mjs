#!/usr/bin/env node
/**
 * One-way migration: MongoDB -> DynamoDB + S3.
 *
...
const { TABLE, getItem, putItem, batchWrite, scanAll } = await import("../util/db/dynamo.js");
const { putMedia, statMedia, isMediaConfigured } = await import("../util/db/media.js");

const URI = process.env.MONGODB_URI;
const DB = process.env.MONGODB_DB || "maharishi";
if (!URI) { console.error("MONGODB_URI is not set — nothing to migrate from"); process.exit(1); }

const strip = ({ _id, ...rest }) => rest;

/**
 * Settings this app actually reads. Anything else stays in Mongo.
 *
 * The cluster still carries a `wholesaleSlug` row and a 91-row `wholesale`
 * collection from the Sankamithra build this repo was forked from. Maharishi
 * is a pure retailer and the whole feature was removed, so copying either would
 * create data no code reads - and a dealer-list address nobody intends to
 * publish. They are left where they are and reported at the end, not deleted:
 * this script does not write to Mongo.
 */
const SETTINGS_KEEP = new Set(["categories", "banner", "priceList"]);
...
const PLAN = {
  products: {
    ...
    pk: "id",
    map: (d) => ({ ...strip(d), id: Number(d.id) }),
  },
  orders: {
    table: TABLE.orders,
    pk: "id",
    map: (d) => ({ ...strip(d), id: String(d.id) }),
  },
  settings: {
    table: TABLE.settings,
    pk: "key",
    map: (d) => ({ ...strip(d), key: String(d.key ?? d._id) }),
    skip: (d) => !SETTINGS_KEEP.has(String(d.key ?? d._id)),
  },
  counters: {
    table: TABLE.counters,
    pk: "name",
    map: (d) => ({ name: String(d._id ?? d.name), seq: Number(d.seq) || 0 }),
  },
};

/** DynamoDB rejects undefined; normalise before writing. */
function clean(o) {
  if (Array.isArray(o)) return o.map(clean);
  if (o && typeof o === "object" && !Buffer.isBuffer(o)) {
    const out = {};
...
console.log(`${VERIFY ? "VERIFY" : WRITE ? "WRITING" : "DRY RUN"}: ${DB} -> DynamoDB (${process.env.AWS_REGION || "ap-south-1"}), prefix "${process.env.DYNAMO_TABLE_PREFIX || "maharishi_"}"\n`);
...
for (const [coll, spec] of Object.entries(PLAN)) {
  ...
  const kept = spec.skip ? docs.filter((d) => !spec.skip(d)) : docs;
  for (const d of docs) if (spec.skip?.(d)) skipped.push(`${coll}.${d.key ?? d._id}`);
  const items = kept.map((d) => clean(spec.map(d)));
  ...
  if (VERIFY) {
    ...
    // Keys, not just counts: two collections of equal size can still differ,
    // and a count alone would call that a clean migration.
    const want = new Set(items.map((i) => String(i[spec.pk])));
    const have = new Set(real.map((i) => String(i[spec.pk])));
    const missing = [...want].filter((k) => !have.has(k));
    const extra = [...have].filter((k) => !want.has(k));
    if (missing.length || extra.length) issues++;
    console.log(`  ${coll.padEnd(10)} mongo ${String(items.length).padStart(4)}  dynamo ${String(real.length).padStart(4)}  ${
      missing.length || extra.length
        ? `MISMATCH${missing.length ? ` missing: ${missing.slice(0, 5).join(",")}` : ""}${extra.length ? ` extra: ${extra.slice(0, 5).join(",")}` : ""}`
        : "ok"}`);
    continue;
  }
  ...
  console.log(`  ${coll.padEnd(10)} ${String(items.length).padStart(4)} docs  ${WRITE ? "written" : "would write"}`);
}
 ...
 */
{
  ...
  if (!docs.length) {
    console.log(`  ${"media".padEnd(10)}    0 docs  (nothing stored)`);
  } else if (!isMediaConfigured()) {
    issues++;
    console.log(`  ${"media".padEnd(10)} ${String(docs.length).padStart(4)} docs  SKIPPED - S3_MEDIA_BUCKET is not set`);
  } else {
    for (const d of docs) {
      ...
      if (VERIFY) {
        ...
        if (!there) issues++;
        console.log(`  ${"media".padEnd(10)} ${String(d.name).slice(0, 28).padEnd(30)} ${mb} MB  ${there ? "ok" : "MISSING"}`);
      } else if (WRITE) {
        await putMedia({ name: d.name, contentType: d.contentType, bytes: Buffer.from(d.data.buffer ?? d.data) });
        console.log(`  ${"media".padEnd(10)} ${String(d.name).slice(0, 28).padEnd(30)} ${mb} MB  -> S3`);
      } else {
        console.log(`  ${"media".padEnd(10)} ${String(d.name).slice(0, 28).padEnd(30)} ${mb} MB  would copy to S3`);
      }
    }
  }
}
...
if (WRITE || VERIFY) {
  ...
  const counter = await getItem(TABLE.counters, "orderRef");
  ...
  if (seq < highest) {
    ...
    if (WRITE) {
      ...
      console.log(`  -> set orderRef to ${highest}; the next bill will be MC-${String(highest + 1).padStart(4, "0")}`);
    } else {
...
/** Left behind on purpose. Said out loud so it is a decision, not an omission. */
{
  const wholesale = await db.collection("wholesale").countDocuments();
  if (wholesale || skipped.length) {
    console.log("\n  left in Mongo, not migrated (the wholesale feature was removed from this app):");
    if (wholesale) console.log(`    wholesale collection — ${wholesale} rows`);
