...
const FROM = (label) =>
  `"${label || process.env.MAIL_NAME || BUSINESS.name}" <${process.env.MAIL_USER}>`;
 ...
 */
function transport() {
  if (globalThis.__maharishiMailer) return globalThis.__maharishiMailer;
  ...
  globalThis.__maharishiMailer = t;
  return t;
}

const SHELL = (title, body) => `
<main style="margin:0;padding:0;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:#1a1a1a;background:#f7f8f9;padding:24px 0;">
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #ededed;">
    <div style="background:#1a4d2e;padding:18px 24px;">
      <div style="color:#fff;font-size:19px;font-weight:800;">${BUSINESS.name}</div>
    ...
    <div style="padding:16px 24px;border-top:1px solid #ededed;color:#7e7e7e;font-size:12px;">
      Questions? Call ${BUSINESS.phone[0]} or reply to this email.
    </div>
  </div>
</main>`;
...
const itemRows = (order) =>
  (order.items || [])
    .map((i) => {
      if (i.substitute) {
        return `<tr><td style="padding:6px 0;border-bottom:1px solid #eee9dd;"><s style="color:#999;">${i.name}</s><br><b>${i.substitute.name}</b> <span style="color:#b26a00;font-size:12px;">(replacement)</span></td><td align="right" style="padding:6px 0;border-bottom:1px solid #eee9dd;">${i.substitute.count} &times; ${inr(i.substitute.unitPrice)}</td></tr>`;
      }
      if (i.unavailable) {
        return `<tr><td style="padding:6px 0;border-bottom:1px solid #eee9dd;color:#999;"><s>${i.name}</s> <span style="font-size:12px;">(out of stock, not supplied)</span></td><td align="right" style="padding:6px 0;border-bottom:1px solid #eee9dd;color:#999;">&mdash;</td></tr>`;
      }
      return `<tr><td style="padding:6px 0;border-bottom:1px solid #eee9dd;">${i.name}</td><td align="right" style="padding:6px 0;border-bottom:1px solid #eee9dd;">${i.count} &times; ${inr(i.unitPrice)}</td></tr>`;
    })
    .join("");
...
export async function sendCustomerMail({ order, invoice, kind = "invoice" }) {
  ...
  const COPY = {
    confirm: {
      subject: `Your ${BUSINESS.name} order ${order.ref} is confirmed`,
    ...
    packed: {
      subject: `Your ${BUSINESS.name} order ${order.ref} is packed and ready`,
      title: "Packed and ready to go",
      lead: `Order <b>${order.ref}</b> is packed and waiting for dispatch. It will be on its way shortly.`,
    },
    cancelled: {
      subject: `Your ${BUSINESS.name} order ${order.ref} has been cancelled`,
      title: "Your order has been cancelled",
      lead: `Order <b>${order.ref}</b> has been cancelled. If this wasn't expected, please call us on ${BUSINESS.phone[0]} and we'll sort it out.`,
    },
  }[kind] || {};
  ...
  const info = {
    ...
    attachments: invoice
      ? [{ filename: `Proforma-${order.ref}.pdf`, content: invoice, encoding: "base64" }]
      : [],
  };
...
export async function sendShopMail({ order, invoice }) {
  ...
  const rows = (order.items || [])
    .map(
      (i) =>
        `<tr><td style="padding:6px 0;border-bottom:1px solid #eee9dd;">${i.name}</td>` +
        `<td align="center" style="padding:6px 0;border-bottom:1px solid #eee9dd;"><b>${i.count}</b></td>` +
        `<td align="right" style="padding:6px 0;border-bottom:1px solid #eee9dd;">${inr(i.total)}</td></tr>`
    )
    .join("");
  ...
  const body = `
    ...
    <div style="padding:14px;background:#f2f6f0;border-radius:8px;margin:0 0 16px;">
      ...
      <div style="margin-top:8px;">
        <a href="tel:${String(c.phone).replace(/\D/g, "")}" style="color:#1a4d2e;font-weight:bold;">${c.phone}</a>
        &nbsp;·&nbsp; ${c.email}
      </div>
    </div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px;margin:0 0 12px;">
      <thead><tr>
        <th align="left" style="padding:6px 0;border-bottom:2px solid #1a1a1a;">Item</th>
        <th align="center" style="padding:6px 0;border-bottom:2px solid #1a1a1a;">Qty</th>
        <th align="right" style="padding:6px 0;border-bottom:2px solid #1a1a1a;">Total</th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
  ...
  const info = {
    from: FROM(`${BUSINESS.name} Orders`),
    to,
    ...
    t.sendMail(
      {
        from: FROM(),
        to: recipient,
        subject: `${BUSINESS.name} mail test`,
        html: SHELL(
          "Mail is working",
