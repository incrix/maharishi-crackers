      ...
      {steps.map((s, i) => {
        ...
        return (
          <Stack key={s.key} direction="row" alignItems="center" flex={i < steps.length - 1 ? 1 : "0 0 auto"} minWidth={0}>
            <Stack direction="row" alignItems="center" gap={1} minWidth={0}>
              <Box
                sx={{
                  ...
                  backgroundColor: active || past ? "var(--primary-color)" : "#eee9dd",
                }}
              >
                {past ? "✓" : i + 1}
              </Box>
            ...
            {i < steps.length - 1 && (
              <Box sx={{ flex: 1, height: 2, mx: 1.25, borderRadius: 9, backgroundColor: past ? "var(--primary-color)" : "#e7e0d0" }} />
            )}
          </Stack>
        );
      })}
    ...
    <Stack gap={{ xs: 2, md: 2.5 }}>
      <Button
        ...
        sx={{
          ...
          color: "var(--primary-color)", "&:hover": { backgroundColor: "#e8f0e4" },
        }}
      >
      ...
      <Stack direction={{ xs: "column", md: "row" }} gap={{ xs: 2, md: 3 }} alignItems="flex-start">
        ...
        <Stack flex={1} gap={2} width="100%" minWidth={0}>
                ...
                {cart.map((row) => (
                  <Stack key={row.id ?? row.name} direction="row" gap={1.5} alignItems="center" pt={1.25}>
                    <Box
                      ...
                      alt=""
                      sx={{ width: 52, height: 52, borderRadius: "8px", objectFit: "cover", flexShrink: 0, backgroundColor: "#f4f0e6" }}
                    />
          ...
          <Stack direction="row" gap={1.25} sx={{ backgroundColor: "#f2f6f0", border: "1px solid #cfe0c8", borderRadius: "12px", p: 2 }}>
...
function OrderPlaced({ result }) {
  ...
  const fileName = `Proforma-${ref || (billing?.name || "order")
    .replace(/[^a-z0-9]+/gi, "-")
    .replace(/^-|-$/g, "")}.pdf`;
        ...
        {received ? (
          <>
            ...
            {ref && (
              <Chip
                label={`Order ${ref}`}
                sx={{ fontWeight: 800, backgroundColor: "#e8f0e4", color: "var(--primary-color)" }}
              />
            )}
