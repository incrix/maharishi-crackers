        ...
        >
          ...
          <Stack gap={1.5}>
            <Stack direction="row" gap={1.25} alignItems="center">
              <Box component="img" src={logo.src} alt={BUSINESS.name} sx={{ height: 54, width: "auto" }} />
              <Stack>
            ...
            <Stack gap={0.75} mt={0.5}>
              {/* Hidden until real - a fake number on every page is worse than none */}
              {!isPlaceholder(BUSINESS.phone[0]) && (
                <ContactLine icon={<PhoneRoundedIcon sx={ic} />} href={`tel:+${BUSINESS.phone[0].replace(/\D/g, "")}`}>
                  {BUSINESS.phone[0]}
                </ContactLine>
              )}
              {!isPlaceholder(BUSINESS.email) && (
                <ContactLine icon={<MailOutlineRoundedIcon sx={ic} />} href={`mailto:${BUSINESS.email}`}>
                  {BUSINESS.email}
                </ContactLine>
              )}
              {hasAddress() ? (
                ...
                <ContactLine icon={<PlaceOutlinedIcon sx={ic} />}>
                  {`${BUSINESS.office.locality}, ${BUSINESS.office.region}`}
                </ContactLine>
              )}
            </Stack>
          </Stack>
