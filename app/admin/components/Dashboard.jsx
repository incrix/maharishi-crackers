...
// Sequential, not categorical: the pipeline is ordered, so it is one hue
// stepped light -> dark rather than four unrelated colours.
const STAGES = [
  { key: "new", label: "New", color: "#b6d6bd" },
  { key: "packing", label: "Packing", color: "#7fb08b" },
  { key: "packed", label: "Packed", color: "#4c9160" },
  { key: "dispatched", label: "Dispatched", color: "#1a4d2e" },
];
            ...
            {STAGES.map((st) => {
              ...
              return (
                <Stack key={st.key} gap={0.5} onClick={() => onJump?.(st.key)} sx={{ cursor: "pointer" }}>
                  ...
                  <Box sx={{ height: 8, borderRadius: 99, backgroundColor: "#eee9dd", overflow: "hidden" }}>
          ...
          <Stack gap={1.25}>
            {top.map((t) => (
              <Stack key={t.name} gap={0.5}>
                ...
                <Box sx={{ height: 8, borderRadius: 99, backgroundColor: "#eee9dd", overflow: "hidden" }}>
                  <Box sx={{ width: `${(t.units / maxUnits) * 100}%`, height: "100%", borderRadius: 99, backgroundColor: "#1a4d2e" }} />
                </Box>
              </Stack>
            ))}
          </Stack>
