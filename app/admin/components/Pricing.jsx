...
/**
 * The price list, one row per product.
 *
 * One list: the website and the counter charge the same rate. A sale
 * percentage, if set, comes off it, and "Sells at" is the result - the figure
 * a customer actually pays either way.
 *
      ...
      <Stack gap={1.25} flexShrink={0} sx={{ minWidth: 0 }}>
        <Stack direction={{ xs: "column", lg: "row" }} gap={1.25} alignItems={{ lg: "center" }}>
          ...
          <Stack direction="row" gap={1} alignItems="center" flexWrap="wrap" sx={{ minWidth: 0 }}>
            <TextField size="small" label="Set sale %" value={bulk}
        ...
        <Stack sx={{ flex: 1, minWidth: 0, minHeight: 0, border: "1px solid var(--border)", borderRadius: "var(--radius)", overflow: "hidden" }}>
          {!compact && (
            <Stack direction="row" gap={1} flexShrink={0}
              ...
              <Th w={100} right>Price ₹</Th><Th w={86} right>Sale %</Th><Th w={100} right>Sells at</Th>
            </Stack>
          )}
          ...
          <Stack sx={{ flex: 1, minWidth: 0, minHeight: 0, overflowY: "auto", overflowX: "hidden", overscrollBehavior: "contain" }}>
            ...
            {visible.map((p) => {
              const price = valueOf(p, "price"), disc = valueOf(p, "discount");
                ...
                return (
                  <Stack key={p.id} gap={1} sx={{ p: 1.25, borderBottom: "1px solid var(--border)",
                    ...
                    <Stack direction="row" gap={1} flexWrap="wrap" sx={{ minWidth: 0 }}>
                      <Field label="Price ₹"><Num value={price} onChange={(v) => setField(p.id, "price", v)} /></Field>
                      <Field label="Sale %"><Num value={disc} onChange={(v) => setField(p.id, "discount", v)} /></Field>
                      <Field label="Sells at"><Readout accent>{inr(p1)}</Readout></Field>
                    </Stack>
                  </Stack>
                );
              }
              ...
              return (
                <Stack key={p.id} direction="row" gap={1} alignItems="center"
                  ...
                  {name}
                  <Box sx={{ width: 100 }}><Num value={price} onChange={(v) => setField(p.id, "price", v)} /></Box>
                  <Box sx={{ width: 86 }}><Num value={disc} onChange={(v) => setField(p.id, "discount", v)} /></Box>
                  <Box sx={{ width: 100, textAlign: "right", pr: 0.5 }}>
                    <Typography fontSize={13} fontWeight={800} color="var(--primary-color)">{inr(p1)}</Typography>
                  </Box>
                </Stack>
              );
            })}
            ...
            <Typography fontSize={11} color="var(--text-color-secondary)" sx={{ px: 1.5, py: 1.25 }}>
              One price list. “Sells at” is the price after any sale percentage — it is
              what both the website and the counter charge.
            </Typography>
          </Stack>
        </Stack>
      )}
