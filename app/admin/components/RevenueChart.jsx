...
export default function RevenueChart({ data, height = 190 }) {
  ...
  // A flat line pinned to the baseline looks like a real (zero) series and
  // invites hovering it. With nothing to plot, say so instead.
      ...
      {!hasRevenue ? (
        ...
        >
          ...
          <Typography fontSize={11.5} color="var(--text-color-trinary)">
            The trend line appears with the first order.
          </Typography>
        </Stack>
      ) : (
      <Box
        ...
        <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" width="100%" height="100%" style={{ display: "block", overflow: "visible" }}>
          <defs>
            <linearGradient id="revFill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="var(--series-counter)" stopOpacity="0.22" />
              <stop offset="100%" stopColor="var(--series-counter)" stopOpacity="0.02" />
            </linearGradient>
          </defs>
          ...
          {[0, 0.5, 1].map((t) => (
            <line key={t} x1="0" x2={W} y1={padY + t * (H - padY * 2)} y2={padY + t * (H - padY * 2)}
              stroke="var(--chart-grid)" strokeWidth="0.4" vectorEffect="non-scaling-stroke" />
          ))}

          <path d={area} fill="url(#revFill)" />
          <path d={line} fill="none" stroke="var(--series-counter)" strokeWidth="2"
            strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />

          {/* Crosshair only. The marker dot is HTML - see below. */}
          {hover != null && data[hover] && (
            <line x1={x(hover)} x2={x(hover)} y1={padY} y2={H} stroke="var(--series-counter)" strokeWidth="1"
              strokeDasharray="3 3" vectorEffect="non-scaling-stroke" opacity="0.5" />
          )}
        </svg>

        {/* Marker: HTML, not SVG, so the stretched viewBox cannot flatten it
            into an ellipse. 2px surface ring so it reads over the line. */}
        {hover != null && data[hover] && (
          <Box
            sx={{
              position: "absolute", pointerEvents: "none",
              left: `${x(hover)}%`,
              top: `${y(data[hover].revenue)}%`,
              transform: "translate(-50%, -50%)",
              width: 10, height: 10, borderRadius: "50%",
              backgroundColor: "var(--series-counter)",
